function registerMessageRoutes(app, deps) {
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (totalBytes > MESSAGE_FILE_LIMITS.maxTotalBytes) {
          removeUploadedFiles(uploadedFiles);
          return res.status(400).json({
            error: `Total upload size cannot exceed ${Math.round(
              MESSAGE_FILE_LIMITS.maxTotalBytes / (1024 * 1024),
            )} MB.`,
          });
        }
  ...
  app.put("/api/messages/:id", (req, res) => {
    ...
    if (!messageId || !username || !String(body || "").trim()) {
      return res.status(400).json({
        error: "Message id, username, and body are required.",
      });
    }
    ...
    const maxMessageChars = Math.max(1, Number(MESSAGE_MAX_CHARS || 4000));
    if (nextBody.length > maxMessageChars) {
      return res.status(400).json({
        error: `Message must be at most ${maxMessageChars} characters.`,
      });
    }

    updateMessageBody(messageId, nextBody);

    emitChatEvent(Number(message.chat_id), {
  ...
  app.delete("/api/messages/:id", (req, res) => {
    ...
    const result = deleteMessageById(messageId);

    if (Array.isArray(result?.storedNames) && result.storedNames.length) {
