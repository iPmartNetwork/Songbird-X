...
export function deleteMessageById(messageId) {
  ...
  if (!targetMessageId) return { deleted: false, chatId: null, storedNames: [] };
  ...
  if (!message) {
    return { deleted: false, chatId: null, storedNames: [] };
  }
  ...
  runWithoutSave("BEGIN");
  try {
    runWithoutSave(
      "DELETE FROM chat_message_reads WHERE message_id = ?",
      [targetMessageId],
    );
    runWithoutSave(
...
export function updateMessageBody(messageId, body) {
  run(
    `
      UPDATE chat_messages
...
export function getMessages(chatId, options = {}) {
  ...
  const params = hasBefore
    ...
    `
    SELECT chat_messages.id, chat_messages.body, chat_messages.created_at, chat_messages.edited_at, chat_messages.read_at, chat_messages.read_by_user_id,
