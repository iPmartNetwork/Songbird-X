...
export function MessageItem({
  ...
  const isEdited = Boolean(msg.edited_at || msg.editedAt);
  ...
  const handleBubbleLongPressCancel = () => {
  ...
  const renderMessageMenu = () => (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="rounded-md px-2 py-1 text-xs text-slate-500 hover:bg-black/5 dark:text-slate-400 dark:hover:bg-white/10"
      ...
      {menuOpen ? (
        <div className="absolute bottom-full right-0 z-30 mb-1 min-w-[148px] rounded-xl border border-slate-200 bg-white p-1 shadow-lg dark:border-white/10 dark:bg-slate-900">
          <button
            type="button"
            onClick={() => {
              setMenuOpen(false);
            ...
            className="flex w-full items-center rounded-lg px-3 py-2 text-left text-xs text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/5"
          >
            Reply
          </button>
          ...
          {isOwn ? (
            <>
              <button
                ...
                className="flex w-full items-center rounded-lg px-3 py-2 text-left text-xs text-sky-600 hover:bg-slate-100 dark:hover:bg-white/5"
              >
                Edit
              </button>
              ...
              <button
                ...
                className="flex w-full items-center rounded-lg px-3 py-2 text-left text-xs text-rose-600 hover:bg-slate-100 dark:hover:bg-white/5"
              >
                Delete
              </button>
            </>
  ...
  const renderFooterMeta = (variant = "default") => {
    const baseTextClass =
      variant === "own"
    ...
    return (
      <div
        className={`mt-2 flex w-full items-center justify-between gap-2 text-[10px] ${baseTextClass}`}
      >
        <div className="flex min-w-0 items-center gap-2">
          <span className="inline-flex items-center gap-1">
            <span>{msg._timeLabel || formatTime(msg.created_at)}</span>
            {isEdited ? <span className="opacity-80">(edited)</span> : null}
        ...
        >
          {!isOwn && isGroupChat && !isChannelChat ? (
            <div className="flex min-w-0 max-w-full items-end gap-2">
              ...
              <div
                data-message-bubble
                className={`relative rounded-2xl rounded-bl-md px-4 py-3 text-sm shadow-sm overflow-visible min-w-0 max-w-[min(76%,calc(100%-2.25rem))] md:max-w-[min(80%,calc(100%-2.25rem))] ${
                  hasFiles
                    ? hasMediaFiles
                      ? "w-[min(52vw,18rem)] md:w-[min(44vw,22rem)] md:min-w-[12rem]"
                      : "w-fit max-w-full"
                    : "max-w-full"
                } bg-white/90 text-slate-800 dark:bg-slate-800/75 dark:text-slate-100`}
                onDoubleClick={() => {
                  if (!isDesktop || !onReply) return;
                  onReply(msg);
                }}
                onTouchStart={handleBubbleLongPressStart}
                onTouchEnd={handleBubbleLongPressCancel}
                onTouchMove={handleBubbleLongPressCancel}
                onTouchCancel={handleBubbleLongPressCancel}
                style={{
              ...
              >
                <button
                  ...
                  className={`mb-1 block max-w-[60vw] truncate text-[11px] font-semibold transition ${
                    ...
                    hasPersian(senderName) ? "font-fa text-right" : "text-left"
                  }`}
            ...
            <div
              ...
              className={`relative rounded-2xl px-4 py-3 text-sm shadow-sm overflow-visible min-w-0 max-w-[78%] sm:max-w-[82%] md:max-w-[75%] ${
                ...
                isOwn
                  ...
                  : "rounded-bl-md bg-white/90 text-slate-800 dark:bg-slate-800/75 dark:text-slate-100"
              }`}
              onDoubleClick={() => {
                if (!isDesktop || !onReply) return;
                onReply(msg);
            ...
            >
              {!isOwn && isGroupChat && !isChannelChat ? (
                <p
                  className={`mb-1 max-w-[60vw] truncate text-[11px] font-semibold sm:max-w-[40vw] md:max-w-[28vw] ${
                    hasPersian(senderName) ? "font-fa text-right" : "text-left"
                  }`}
                  style={{ color: String(senderColor) }}
                  dir="auto"
              ...
              {renderFooterMeta(isOwn ? "own" : "other")}
            </div>
          )}
        </div>
      ) : null}
