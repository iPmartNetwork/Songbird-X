...
const NOTIFICATIONS_ENABLED_KEY = "songbird-x-notify-enabled";
const OPEN_CHAT_ID_KEY = "songbird-x-open-chat-id";
const PRESENCE_IDLE_THRESHOLD_MS = 12 * 1000;
const CHAT_CACHE_VERSION = 2;
const CHAT_LIST_CACHE_KEY = "songbird-x-chat-list-cache";
const CHAT_MESSAGES_CACHE_KEY = "songbird-x-chat-messages-cache";
const CHAT_MESSAGES_INDEX_KEY = "songbird-x-chat-messages-index";
...
const CHANNEL_SEEN_CACHE_KEY = "songbird-x-channel-seen";
const MESSAGE_CACHE_MAX = Math.max(
  50,
  Math.min(120, CHAT_PAGE_CONFIG.messageFetchLimit),
);
...
const canUseLocalStorage = () => {
  ...
  try {
    const testKey = "__songbird_x_ls_test__";
...
const sanitizeMessagesForCache = (messages) => {
  ...
  const result = [];
  for (let i = messages.length - 1; i >= 0 && result.length < MESSAGE_CACHE_MAX; i -= 1) {
    const msg = messages[i];
    if (isCacheableMessage(msg)) {
      result.unshift(sanitizeMessageForCache(msg));
    }
  }
  return result;
};
...
const normalizeMessagesForRender = (messages) => {
  ...
  return messages.map((msg) => {
    ...
    return normalizeMessageForRender(msg);
  });
};
...
export default function ChatPage({ user, setUser, isDark, setIsDark, toggleTheme }) {
  ...
  const [messageActionLoadingId, setMessageActionLoadingId] = useState(null);
  ...
  const handleDeleteMessage = useCallback(async (msg) => {
    const messageId = Number(msg?.id || msg?._serverId || 0);
    if (!messageId) return;
    ...
    try {
      ...
      setMessages((prev) =>
        prev.filter(
          (item) => Number(item?.id || item?._serverId || 0) !== messageId,
        ),
      );
      ...
      window.alert(String(error?.message || "حذف پیام انجام نشد."));
    }
  }, [user.username, scrollChatToBottom]);

  const handleEditMessage = useCallback(async (msg) => {
    const messageId = Number(msg?.id || msg?._serverId || 0);
    if (!messageId) return;

    const currentBody = normalizeMessageBody(msg?.body);
    const nextBody = window.prompt("Edit message:", currentBody);

    if (nextBody === null) return;

    const trimmed = String(nextBody).trim();
    if (!trimmed) {
      window.alert("متن پیام نمی‌تواند خالی باشد.");
      return;
    }

    if (trimmed.length > Number(APP_CONFIG.messageMaxChars || CHAT_PAGE_CONFIG.messageMaxChars || 4000)) {
      window.alert("متن پیام بیش از حد مجاز است.");
      return;
    }
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((item) => {
          const currentId = Number(item?.id || item?._serverId || 0);
          if (currentId !== messageId) return item;
          return {
            ...item,
            body: trimmed,
            edited_at: editedAt,
      ...
      window.alert(String(error?.message || "ویرایش پیام انجام نشد."));
    }
  }, [user.username]);
  ...
  useEffect(() => {
    ...
    document.title =
  totalUnreadCount > 0
    ? `Songbird-X | ${totalUnread} new message${totalUnread === 1 ? "" : "s"}`
    : "Songbird-X";;
